...
class DatabaseMigrationPrerequisiteStack(core.Stack):
    ...
    def __init__(
        ...
        **kwargs
        ...
        dms_sg.add_ingress_rule(
            peer=_ec2.Peer.ipv4(vpc.vpc_cidr_block),
            connection=_ec2.Port.tcp(1521),
            description="Allow Oracle inbound connetions"
        )
        dms_sg.add_ingress_rule(
            peer=_ec2.Peer.ipv4(vpc.vpc_cidr_block),
            connection=_ec2.Port.tcp(6379),
            description="Allow Redis inbound connetions"
        )
        ...
        # Create Security Group for Oracle Server Instance
        postgresql_db_sg = _ec2.SecurityGroup(
            self,
            id="PostgreSqlSecurityGroup",
            vpc=vpc,
            security_group_name=f"postgresql_db_sg_{id}",
            description="Security Group for PostgreSQL"
        )

        postgresql_db_sg.add_ingress_rule(
            peer=_ec2.Peer.ipv4(vpc.vpc_cidr_block),
            connection=_ec2.Port.tcp(5432),
            description="Allow PostgreSQL inbound connetions"
        )
        # Create Security Group for Elasticache Instance
        elasticache_db_sg = _ec2.SecurityGroup(
            self,
            id="ElasticacheSecurityGroup",
            vpc=vpc,
            security_group_name=f"elasticache_db_sg_{id}",
            description="Security Group for Elasticache"
        )

        elasticache_db_sg.add_ingress_rule(
            peer=_ec2.Peer.ipv4(vpc.vpc_cidr_block),
            connection=_ec2.Port.tcp(6379),
            description="Allow Elasticache inbound connetions"
        )
